...
public partial class TypesenseSearcherTests
{
    ...
    public async Task CanRetrieveObjectTypes()
    {
        ...
        Assert.Multiple(
            () =>
            {
                ...
                Assert.That(
                    documents.SingleOrDefault(d => d.Id == _documentIds[1])?.ObjectType,
                    Is.EqualTo(UmbracoObjectTypes.Document)
                );
                Assert.That(
                    documents.SingleOrDefault(d => d.Id == _documentIds[26])?.ObjectType,
                    Is.EqualTo(UmbracoObjectTypes.Media)
                );
                Assert.That(
                    documents.SingleOrDefault(d => d.Id == _documentIds[51])?.ObjectType,
                    Is.EqualTo(UmbracoObjectTypes.Member)
                );
                Assert.That(
                    documents.SingleOrDefault(d => d.Id == _documentIds[76])?.ObjectType,
                    Is.EqualTo(UmbracoObjectTypes.Unknown)
                );
            }
        );
    }
    ...
    public async Task CanCombineFacetsWithinFields()
    {
        ...
        for (var i = 0; i < 100; i++)
        {
            Assert.Multiple(
                () =>
                {
                    IntegerExactFacetValue? integerFacetValue = integerFacetValues.FirstOrDefault(v => v.Key == i + 1);
                    Assert.That(integerFacetValue?.Count, Is.EqualTo(1));
    ...
    public async Task CanHaveSameTypeFacetsWithinFields()
    {
        ...
        for (var i = 0; i < 100; i++)
        {
            IntegerExactFacetValue? integerFacetValue = integerExactFacetValues.FirstOrDefault(v => v.Key == i + 1);
            Assert.That(integerFacetValue?.Count, Is.EqualTo(1));
        }
    }
        ...
        Assert.Multiple(
            () =>
            {
                ...
                Assert.That(
                    ...
                    Is.EquivalentTo(_documentIds.Values.Skip(5).Take(5))
                );
            }
        );
    }
            ...
            [
                ...
                new IntegerRangeFacet(
                    ...
                    [
                        ...
                        new IntegerRangeFacetRange("two", 5, 10),
                        new IntegerRangeFacetRange("three", 10, 20),
                        new IntegerRangeFacetRange("four", 20, 25),
                    ]
                )
            ]
        );
